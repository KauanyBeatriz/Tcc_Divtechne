...
def load_profissional(id_profissional):
    # Buscando por query na classe usuario fazendo um get no id do usario que é int
    return Profissional.query.get(int(id_profissional))
...
class Cliente(database.Model, UserMixin):
    ...
    chat_cliente = database.relationship("Chat", backref="cliente", lazy=True)
    contato_cliente = database.relationship("Contato", backref="cliente", lazy=True)
    proposta_cliente = database.relationship("Proposta", backref="cliente", lazy=True)
...
class Vaga(database.Model):
    ...
    candidatar_vaga = database.relationship("CandidatoVaga", backref="vaga", lazy=True)
...
class Servico(database.Model):
    ...
    candidatar_servico = database.relationship("CandidatoServico", backref="servico", lazy=True)
...
class Projeto(database.Model):
    ...
    candidatar_projeto = database.relationship("CandidatoProjeto", backref="projeto", lazy=True)


# Tabela de Profissional
class Profissional(database.Model, UserMixin):
    id = database.Column(database.Integer, primary_key=True)
    ...
    chat_profissional = database.relationship("Chat", backref="profissional", lazy=True)
    contato_profissional = database.relationship("Contato", backref="profissional", lazy=True)
    candidato_vaga = database.relationship("CandidatoVaga", backref="profissional", lazy=True)
    candidato_servico = database.relationship("CandidatoServico", backref="profissional", lazy=True)
    candidato_projeto = database.relationship("CandidatoProjeto", backref="profissional", lazy=True)
    proposta_profissional = database.relationship("Proposta", backref="profissional", lazy=True)
...
class Chat(database.Model):
    ...
    anexo = database.Column(database.String)
    # Relacionamento
    id_cliente_chat = database.Column(database.Integer, database.ForeignKey("cliente.id"), nullable=False)
    id_programador_chat = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)


class Contato(database.Model):
    id = database.Column(database.Integer, primary_key=True)
    # Relacionamento
    id_cliente_contato = database.Column(database.Integer, database.ForeignKey("cliente.id"), nullable=False)
    id_programador_contato = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)


class CandidatoVaga(database.Model):
    id = database.Column(database.Integer, primary_key=True)
    # Relacionamento
    id_vaga_candidato = database.Column(database.Integer, database.ForeignKey("vaga.id"), nullable=False)
    id_programador_candidato = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)


class CandidatoServico(database.Model):
    id = database.Column(database.Integer, primary_key=True)
    # Relacionamento
    id_servico_candidato = database.Column(database.Integer, database.ForeignKey("servico.id"), nullable=False)
    id_programador_candidato = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)


class CandidatoProjeto(database.Model):
    id = database.Column(database.Integer, primary_key=True)
    # Relacionamento
    id_projeto_candidato = database.Column(database.Integer, database.ForeignKey("projeto.id"), nullable=False)
    id_programador_candidato = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)


class Proposta(database.Model):
    id = database.Column(database.Integer, primary_key=True)
    contato = database.Column(database.String, nullable=False)
    proposta = database.Column(database.Text, nullable=False)
    contrato = database.Column(database.String)
    # Relacionamentos
    id_cliente_proposta = database.Column(database.Integer, database.ForeignKey("cliente.id"), nullable=False)
    id_programador_proposta = database.Column(database.Integer, database.ForeignKey("profissional.id"), nullable=False)